...
* We are **not** creating feature from code but we use existing 🚀 **[Extrude feature](/solidworks-macros/feature-create-extrude-boss)** to create **Wrap feature**.
...
* If selected `object` is **not** type of *SOLIDWORKS Sketch* then 🚀 **[Else statement](/vba/vba-if-then-structure-select-case/)** code runs.
