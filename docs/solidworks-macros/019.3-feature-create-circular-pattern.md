...
* We are **not** creating feature from code but we use existing 🚀 **[Extrude feature](/solidworks-macros/feature-create-extrude-boss)** and 🚀 **[Hole feature](solidworks-macros/feature-create-hole-wizard/)** to create **Circular pattern feature**.
...
* 🚀 **[IF statement](/vba/vba-if-then-structure-select-case/)**  is `True` when selected `object` is type of *SOLIDWORKS Edge*.
* If selected `object` is **not** type of *SOLIDWORKS Edge* then 🚀 **[Else statement](/vba/vba-if-then-structure-select-case/)** code runs.
